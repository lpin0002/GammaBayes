...
def DM_spectrum_setup(logmDM=-0.7, lambdainput=0.1, eaxis=np.logspace(-6, 4, 3001)):
    ...
    def dm_fullspec(logmDM=logmDM):
        ...
        stopindex = np.where(np.log10(eaxis)>=logmDM)[0][0]
        stopindex+=1
        if stopindex<2:
            stopindex=2
        
        newyvals = np.log(yvals)[:stopindex]
        newlog10eaxis = np.log10(eaxis)[:stopindex]
                
        norm = special.logsumexp(newyvals+np.log(10**newlog10eaxis)+np.log(np.log(10))+np.log(newlog10eaxis[1]-newlog10eaxis[0]))

        if np.isneginf(norm) or np.isnan(norm):
            norm=0 
        
        fullspectrum = interpolate.interp1d(y=newyvals-norm, x =newlog10eaxis, kind='linear',
