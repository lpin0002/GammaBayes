...
def sigmargsetup(edisplist):
       def sigmarg(logmass, eaxis_mod=eaxis_mod, axis=axis):
              ...
              return tempmarglogmassrow
       
       return sigmarg
       


def sigmargpool(edisplist, logmassrange=logmassrange):
       sigmarglogzvals = []
       sigmargfunc = sigmargsetup(edisplist)
       
       with Pool(8) as pool:
              results = pool.map(sigmargfunc, logmassrange)
...
if __name__ == '__main__':
       ...
       sigmarglogzvals = sigmargpool(edisplist=edisplist, logmassrange=logmassrange)
